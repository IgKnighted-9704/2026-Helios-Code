...
import frc.robot.Constants.SubsystemConstants.ClimbSubsystemConstants;
...
public class ClimbSubsystem {
    ...
    //Drivetrain
        private TunerSwerveDrivetrain swerve;
        
    public ClimbSubsystem(TunerSwerveDrivetrain swerve){
        swerve = this.swerve;
        robotPivotAngle = swerve.getPigeon2().getPitch().getValueAsDouble();
    ...
    //Utility Tools
        private double getClimbInches(){
            ...
            ((climbMotorA.getPosition().getValueAsDouble() * (2*Math.PI)
                * ClimbSubsystemConstants.CLIMB_WHEEL_RADIUS_INCHES) + 
            (climbMotorB.getPosition().getValueAsDouble() * (2*Math.PI)
                * ClimbSubsystemConstants.CLIMB_WHEEL_RADIUS_INCHES))/2;
        }
